...
public class ExcelReader {
    ...
    public static void main(String[] args) {
        ...
        // 读取Excel文件内容
        List<Admin> readResult = ExcelReader.readExcel(excelFileName);
     ...
     */
    public static List<Admin> readExcel(MultipartFile file) {
        ...
        try {
            ...
            // 读取excel中的数据
            List<Admin> resultDataList = parseExcel(workbook);
     ...
     * @return 读取结果列表，读取失败时返回null
     */
    public static List<Admin> readExcel(String fileName) {
     ...
     * @return 解析结果
     */
    private static List<Admin> parseExcel(Workbook workbook) {
       List<Admin> resultDataList = new ArrayList<>();
        // 解析sheet
        for (int sheetNum = 0; sheetNum < workbook.getNumberOfSheets(); sheetNum++) {
            ...
            for (int rowNum = rowStart; rowNum < rowEnd; rowNum++) {
                ...
                Admin resultData = convertRowToData(row);
                if (null == resultData) {
     ...
     */
    private static Admin convertRowToData(Row row) {
        ...
        resultData.setCreateName(location);
        // 获取职业
        cell = row.getCell(cellNum++);
        String job = convertCellValueToString(cell);
        resultData.setAvatar(job);
